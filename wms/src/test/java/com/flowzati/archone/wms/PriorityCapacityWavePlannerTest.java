...
class PriorityCapacityWavePlannerTest {
  ...
  void filtersHardConstraintsThenGreedilyFillsRemainingCapacityInStablePriorityOrder() {
    ...
    WavePlanningPolicy policy = new WavePlanningPolicy(
        FACILITY_ID, T0.plusSeconds(1_000), 2, 3, 5);

    var forward = planner.plan(
        List.of(smallLowerPriority, outsideCutoff, tooLargeForRemainder,
            anotherFacility, largeHighPriority),
        policy);
    var reverse = planner.plan(
        List.of(largeHighPriority, anotherFacility, tooLargeForRemainder,
            outsideCutoff, smallLowerPriority),
        policy);

    assertThat(forward).extracting(assignment -> assignment.shipmentId())
        .containsExactly(largeHighPriority.shipmentId(), smallLowerPriority.shipmentId());
    assertThat(reverse).isEqualTo(forward);
    assertThat(forward.stream().mapToInt(assignment -> assignment.unitCount()).sum()).isEqualTo(5);
    assertThat(forward.stream().mapToInt(assignment -> assignment.lineCount()).sum()).isEqualTo(3);
  }
