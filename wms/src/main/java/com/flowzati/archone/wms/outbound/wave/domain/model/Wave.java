package com.flowzati.archone.wms.outbound.wave.domain.model;
...
public class Wave {
  ...
  public void release(int warehouseWorkCount, int pickTaskCount, Instant releasedAt) {
    ...
    events.add(new WaveReleased(
        id, facilityId, assignments.size(), warehouseWorkCount, pickTaskCount, releasedAt));
  }
  ...
  private static void requireWithinPolicy(
    ...
    int totalLines = assignments.stream()
        .map(WaveAssignment::lineCount)
        .reduce(0, Math::addExact);
    int totalUnits = assignments.stream()
        .map(WaveAssignment::unitCount)
        .reduce(0, Math::addExact);
