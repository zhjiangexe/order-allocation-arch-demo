...
public class ConfirmPickUsecase {
  ...
  public void handle(ConfirmPickCommand command) {
    Shipment shipment = shipmentRepository.findByPickTaskId(command.pickTaskId())
        .orElseThrow(() -> new IllegalStateException("Shipment for pick task not found: " + command.pickTaskId()));
