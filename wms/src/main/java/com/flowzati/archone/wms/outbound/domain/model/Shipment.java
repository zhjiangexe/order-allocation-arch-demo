...
public class Shipment {
  ...
  public void confirmPick(UUID pickTaskId, int actualQuantity, Instant confirmedAt) {
    ...
    PickTask task = pickingWork.pickTasks().stream()
        ...
        .orElseThrow(() -> new IllegalArgumentException(
            "Pick task does not belong to Shipment: " + pickTaskId));
    ...
    if (task.status() == PickTaskStatus.SHORT_PICKED) {
      events.add(new ShortPickDetected(
          id, task.id(), task.moveId(), task.skuCode(), task.sourceLocationId(),
          task.requestedQuantity(), task.pickedQuantity(), confirmedAt));
      return;
    }
  ...
  public void handOverToCarrier(Instant handedOverAt) {
    ...
    requireStatus(
        ...
        "Only a shipment ready for dispatch can be handed over");
  ...
  private void requireWorkMatchesLines(WarehouseWork work) {
    boolean everyLineHasMatchingTask = lines.stream().allMatch(line ->
        work.pickTasks().stream().anyMatch(task ->
            task.orderLineId().equals(line.orderLineId())
  ...
  public boolean isPickingWorkTerminal() {
    return status == ShipmentStatus.CANCELLED
        || (pickingWork != null && pickingWork.isTerminal());
  }
