...
- **主檔要不要一支扁平的查詢端點？** 操作台把 `ownerId` 與 SKU 代碼還原成看得懂的字，靠的是
  進場載入整份主檔，而那是三層巢狀端點的 fan-out：`1 + N貨主 + Σ款`。本 change 的 demo 資料
  （2 貨主、3 款）是 6 支請求，可接受；但這個數字隨主檔線性成長，而它是下單表單能用的前提。

  要收掉得由後端提供一支扁平的主檔查詢（例如一次回攤平的 owner／product／sku），把 6 支收成
  1 支。**本 change 不做**：這是效能問題不是正確性問題，現有端點的資源劃分（款與規格巢狀在
  貨主之下，因為編碼跨貨主撞號、貨主是它們存在的前提）是對的，扁平端點是額外的讀取視圖而
  不是取代它們。等主檔規模真的讓這個 fan-out 會痛時再做，屆時要一併決定它與三層端點如何並存。
