...
- 將配貨完成結果抽象為 allocation context 內部的通用 allocation completion fact；第一階段由 order adapter 繼續發布既有 `OrderAllocatedIntegrationEvent` v1 與 `AllocationCommittedForFulfillmentIntegrationEvent` v1，不直接破壞既有 wire contract。新來源於後續 change 定義自己的 integration contract。
...
- 新增內部 generic allocation completion fact 與 order adapter translation，但保留現有 order/fulfillment v1 integration event payload、event type 與欄位語意；尤其 fulfillment v1 `allocationId` 仍是 `pickingId`，不得改成新的 `allocationDemandId`。
