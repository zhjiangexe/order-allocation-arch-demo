...
回頭看訂單轉為 `ALLOCATED`——只能靠 curl 或 IDE 的 HTTP client，無法在螢幕分享時
讓觀看者跟上。
...
- 兩個路由頁面：訂單頁（下單表單 ＋ 最近訂單列表）與庫存頁（SKU 查詢 ＋ 補貨探針）。
  四個功能收斂為兩頁，因為「下單→看列表」與「查庫存→補貨→再查」各自是連續動作，
  拆開會讓一次操作跨頁跳轉。
- 共用頁首顯示目前生效的分區策略，讓觀看者知道跑的是 v1 還是 v3。
- 訂單列表直接攤開訂單表示的全部欄位（狀態與四個階段時間戳），不做詳細頁或 modal
  ——那會是同一份資料的第二次呈現。
...
- 不做訂單詳細頁或 modal，也不呈現事件因果鏈；後端沒有提供，而列表已含訂單的全部欄位。
...
- `demo-console-frontend`: 本機執行的兩頁操作台，讓下單、查最近訂單、查庫存、觸發
  補貨這四件事可以在瀏覽器中完成與觀察，且所有資料取得皆可追溯到明確的使用者動作。
