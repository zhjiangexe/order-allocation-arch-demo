...
| `PendingPickingBacklogAssignmentUsecase` | `StockOperationBacklogInteractor` |
...
`AssignmentQueueKey` 直接作為 availability/backlog 入口的 transport-neutral value object；移除只重複其欄位的 `AssignPendingPickingCommand` 與只包一層 transaction 的 `PendingPickingAssignmentUsecase`。Scheduler 使用 `StockOperationBacklogInteractor` 表達「尋找並補償尚未 assignment 的 operation」，reconciler 仍逐一呼叫同一 façade，不複製 planner/commit 流程。
