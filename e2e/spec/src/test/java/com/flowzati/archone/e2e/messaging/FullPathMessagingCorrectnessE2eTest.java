...
class FullPathMessagingCorrectnessE2eTest {
    ...
    void shouldRollbackEveryFailedAttemptPreserveDltMetadataAndRecoverByReplay() {
        ...
        try (KafkaConsumer<String, String> orderProbe = probeAtEnd(ORDER_EVENTS);
            ...
            UUID placedEventId = awaitOutboxEvent(orderId, OrderPlacedIntegrationEvent.EVENT_TYPE);
            ConsumerRecord<String, String> original = awaitEvent(orderProbe, placedEventId, NORMAL_FLOW_TIMEOUT);
            ConsumerRecord<String, String> deadLetter = awaitEvent(dltProbe, placedEventId, FAILURE_FLOW_TIMEOUT);
    ...
    private static void assertDeadLetterPreserves(
            ConsumerRecord<String, String> original, ConsumerRecord<String, String> deadLetter) {
        ...
        assertThat(textHeader(deadLetter, "eventType")).isEqualTo(textHeader(original, "eventType"));
        assertThat(textHeader(deadLetter, "messageHeaders")).isEqualTo(textHeader(original, "messageHeaders"));
        assertThat(textHeader(deadLetter, KafkaHeaders.DLT_ORIGINAL_TOPIC)).isEqualTo(original.topic());
        assertThat(intHeader(deadLetter, KafkaHeaders.DLT_ORIGINAL_PARTITION)).isEqualTo(original.partition());
        assertThat(longHeader(deadLetter, KafkaHeaders.DLT_ORIGINAL_OFFSET)).isEqualTo(original.offset());
