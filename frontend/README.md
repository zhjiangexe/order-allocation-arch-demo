...
**庫存頁**——查某貨主某 SKU 的**全部批次**，並對同一個組合觸發補貨。兩者共用貨主與 SKU
欄位，因為它們是一個連續動作：查到可承諾量是 0、補一批、再查一次。

批次列表逐批顯示倉別、入庫日、效期、三個數量與狀態，**順序就是配貨會取用的順序**（FEFO：
效期近的先出，同效期則先進先出）——所以第一列就是下一張單會吃到的那一批。

**過期的批會列出來並標記，不是濾掉。** 「有 100 件但一件都出不了」與「什麼都沒有」要引導出
不同的動作（報廢 vs 進貨），濾掉就分不出來了。同理，沒過期但被預留光的批標成「已預留完」
而不是與過期共用一個「不可配」——配貨眼中兩者相同，但操作上一個要報廢、一個只是等出貨。

**補貨要填五個維度**：貨主、倉別、SKU、入庫日、效期。它們合起來決定這批貨加到哪一列——命中
既有列就加數量，否則新開一列。缺任一個就得定義合併規則，而任何一條規則都會在某些情況下把
不可互換的貨併在一起，所以五個都必填、缺一個表單就不放行。
...
1. 庫存頁選乙貨主、查 `SKU-EMPTY` → 一列批次，在手 0、可承諾 0
2. 訂單頁對它下幾張單 → 看到 `PENDING`，按重新整理 → 變成 `BACKORDERED`
3. 庫存頁對它補貨（倉別選南部倉，入庫日與效期隨意）→ 顯示「已受理」與事件識別碼
   （**不是**「已配置」）
4. 訂單頁按重新整理 → 看到那批訂單依 FIFO 轉成 `ALLOCATED`、`allocatedAt` 有值
5. 回庫存頁再查一次 → 那批的已預留量上去了，而**如果補貨的入庫日或效期與既有的那列不同，
   會看到多出一列**——那正是「五個維度全等才是同一批」

另一條值得走的流程：甲貨主的 `SKU-AVAILABLE` 種子有四批，其中兩批同效期不同入庫日、一批
已過期。查它可以一次看到 FEFO 的排序、同效期的先後、以及「有貨但已過期」三件事。
