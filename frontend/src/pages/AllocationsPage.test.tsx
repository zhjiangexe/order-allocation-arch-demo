...
it('combines owner, facility and source filters without fetching a different queue', async () => {
  ...
  fireEvent.change(screen.getByLabelText('來源'), { target: { value: 'TRANSFER' } });
  expect(screen.getAllByRole('article')).toHaveLength(1);
  fireEvent.change(screen.getByLabelText('貨主'), { target: { value: order.ownerId } });
  expect(screen.getByText('已載入的作業中沒有符合篩選的資料。')).toBeVisible();
  fireEvent.change(screen.getByLabelText('來源'), { target: { value: 'ORDER' } });
  fireEvent.change(screen.getByLabelText('設施'), { target: { value: order.facilityId } });
  expect(screen.getAllByRole('article')).toHaveLength(1);
