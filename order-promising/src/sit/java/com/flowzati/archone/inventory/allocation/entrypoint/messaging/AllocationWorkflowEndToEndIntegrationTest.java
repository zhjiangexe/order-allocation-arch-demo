...
class AllocationWorkflowEndToEndIntegrationTest {
  ...
  void shouldAllocateOrderFromKafkaIntegrationEventAndWriteOutbox() throws Exception {
    ...
    stockQuantRepository.save(StockFixtures.unexpiredBatch(stockQuantId, "SKU-AVAILABLE", 10, 0));
  ...
  void shouldReleaseActiveReservationFromKafkaCancellationEvent() throws Exception {
    ...
    stockQuantRepository.save(
        StockFixtures.unexpiredBatch(stockQuantId, "SKU-PARTIALLY-RESERVED", 10, 4));
    // 搬運指向**行**（外鍵 fk_stock_moves_order_line），而單據指向訂單——取消要從單據找起。
    MovementFixtures.seedAssignedPicking(jdbcTemplate, order, stockQuantId, 4);
  ...
  void shouldAllocateOnlyFifoPrefixAfterReceipt() throws Exception {
    ...
    stockQuantRepository.save(StockFixtures.unexpiredBatch(stockQuantId, "SKU-FIFO", 0, 0));
  ...
  void shouldReserveNothingWhenOneSkuOfAMultiSkuOrderFallsShort() throws Exception {
    ...
    stockQuantRepository.save(StockFixtures.unexpiredBatch(plentifulId, "SKU-BASKET-A", 100, 0));
    stockQuantRepository.save(StockFixtures.unexpiredBatch(scarceId, "SKU-BASKET-B", 3, 0));
    ...
    assertThat(orderRepository.findById(orderId)).hasValueSatisfying(order ->
        assertThat(order.getStatus()).isEqualTo(OrderStatus.PENDING));
    assertThat(heldBy(orderId)).isEmpty();
    assertThat(stockQuantRepository.findById(plentifulId)).hasValueSatisfying(pool ->
        assertThat(pool.getReservedQuantity()).isZero());
    assertThat(stockQuantRepository.findById(scarceId)).hasValueSatisfying(pool ->
        assertThat(pool.getReservedQuantity()).isZero());
  }
  ...
  void shouldAllocateTheWholeBasketWhenEverySkuIsCovered() throws Exception {
    ...
    orderRepository.save(OrderFixtures.pendingMultiSkuOrder(orderId, receivedAt,
        new java.util.LinkedHashMap<>(java.util.Map.of("SKU-BASKET-A", 10, "SKU-BASKET-B", 5))));
    stockQuantRepository.save(StockFixtures.unexpiredBatch(firstPoolId, "SKU-BASKET-A", 100, 0));
    stockQuantRepository.save(StockFixtures.unexpiredBatch(secondPoolId, "SKU-BASKET-B", 100, 0));
    ...
    assertThat(stockQuantRepository.findById(firstPoolId)).hasValueSatisfying(pool ->
        assertThat(pool.getReservedQuantity()).isEqualTo(10));
    assertThat(stockQuantRepository.findById(secondPoolId)).hasValueSatisfying(pool ->
  ...
  void shouldCountTwoLinesOfTheSameSkuOnceAsTheirSum() throws Exception {
    ...
    orderRepository.save(order);
    stockQuantRepository.save(StockFixtures.unexpiredBatch(poolId, "SKU-BASKET-A", 10, 0));
    ...
    assertThat(stockQuantRepository.findById(poolId)).hasValueSatisfying(pool ->
        assertThat(pool.getReservedQuantity()).isEqualTo(10));
    assertThat(heldBy(orderId)).hasSize(2);
  }
  ...
  void shouldJudgeACandidatesOtherSkuAgainstItsOwnStock() throws Exception {
    ...
    stockQuantRepository.save(StockFixtures.unexpiredBatch(poolId, "SKU-BASKET-A", 0, 0));
