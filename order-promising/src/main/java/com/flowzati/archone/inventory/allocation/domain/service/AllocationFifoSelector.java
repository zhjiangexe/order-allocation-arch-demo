...
public class AllocationFifoSelector {
    ...
    private static boolean isHeadOfEveryRequiredSku(AllocationDemand candidate, AllocationCandidateBatch batch) {
        return candidate.totalsBySku().keySet().stream().allMatch(skuCode -> isQueueHead(candidate, skuCode, batch));
    }

    private static boolean isQueueHead(AllocationDemand candidate, String skuCode, AllocationCandidateBatch batch) {
