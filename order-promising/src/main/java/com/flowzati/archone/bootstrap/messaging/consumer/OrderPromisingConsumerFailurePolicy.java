...
final class OrderPromisingConsumerFailurePolicy {
  ...
  static final int MAX_TRANSIENT_INFRASTRUCTURE_PROCESSING_ATTEMPTS =
      KAFKA_REDELIVERY_RETRIES + 1;

  private OrderPromisingConsumerFailurePolicy() {
  }

  static OptimisticLockingRetrySettings optimisticLockingRetrySettings() {
    return new OptimisticLockingRetrySettings(
        LOCAL_OPTIMISTIC_RETRIES, LOCAL_OPTIMISTIC_RETRY_DELAY);
  }

  static KafkaConsumerFailurePolicy kafkaConsumerFailurePolicy() {
    ExponentialBackOffWithMaxRetries retryBackOff =
        new ExponentialBackOffWithMaxRetries(KAFKA_REDELIVERY_RETRIES);
  ...
  private static TypeBasedMessageFailureClassifier failureClassifier() {
    return TypeBasedMessageFailureClassifier.builder()
        ...
        .nonRetryable(
            IntegrationEventContractException.class,
            MessageFailureCategory.CONTRACT)
