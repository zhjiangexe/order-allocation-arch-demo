...
public record AcceptAllocationDemandCommand(
  ...
  public AcceptAllocationDemandCommand {
    if (source == null || ownerId == null || facilityId == null || sourceLocationId == null
        || requiredBy == null || enqueuedAt == null || executionIntent == null) {
  ...
  public List<AllocationDemandLineRequest> demandLines() {
    return lines.stream().map(line -> new AllocationDemandLineRequest(
        line.sourceLineId(), line.skuCode(), line.quantity())).toList();
  }

  public record SourceDemandLine(
      String sourceLineId,
      String skuCode,
      int quantity,
      UUID sourceLineReferenceId
  ) {
