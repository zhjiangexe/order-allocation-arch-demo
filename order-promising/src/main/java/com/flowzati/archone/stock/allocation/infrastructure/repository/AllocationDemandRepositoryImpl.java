...
import com.flowzati.archone.stock.allocation.infrastructure.mapper.AllocationDemandMapper;
...
public class AllocationDemandRepositoryImpl implements AllocationDemandRepository {
  ...
  public Optional<AllocationDemand> findBySource(SourceAllocationUnit source) {
    return repository.findBySourceTypeAndSourceIdAndAllocationUnitKey(
  ...
  public AllocationCandidateBatch findPendingCandidates(
    ...
    List<UUID> contextIds = repository.findFifoContextIds(
        scope.ownerId(), scope.facilityId(), scope.locationId(), candidateIds);
    Comparator<AllocationDemand> precedence = Comparator
    ...
    List<AllocationDemand> candidates = candidateIds.stream().map(byId::get)
        .sorted(precedence).toList();
    List<AllocationDemand> context = byId.values().stream().sorted(precedence).toList();
    return new AllocationCandidateBatch(candidates, context);
  }

  @Override
  public List<WaitingAllocationScope> findAllocatablePendingScopes(
      java.time.LocalDate today, int limit) {
