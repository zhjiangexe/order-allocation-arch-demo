...
import com.flowzati.archone.stock.allocation.infrastructure.entity.AllocationDemandEntity;
import com.flowzati.archone.stock.allocation.infrastructure.entity.AllocationDemandLineEntity;
  ...
  private static List<AllocationDemandLine> toDomainLines(AllocationDemandEntity demand) {
    return demand.getLines().stream()
        .map(line -> new AllocationDemandLine(
            line.getId(), demand.getId(), line.getSourceLineId(), line.getSkuCode(),
            line.getQuantity(), line.getLineSequence()))
        .toList();
  }
}
