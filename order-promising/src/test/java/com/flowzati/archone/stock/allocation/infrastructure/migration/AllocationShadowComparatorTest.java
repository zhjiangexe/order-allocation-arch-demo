package com.flowzati.archone.stock.allocation.infrastructure.migration;

import static org.assertj.core.api.Assertions.assertThat;

import com.flowzati.archone.stock.allocation.domain.valueobject.SourceAllocationUnit;
import com.flowzati.archone.stock.allocation.infrastructure.migration.AllocationShadowComparator.DemandSnapshot;
import com.flowzati.archone.stock.allocation.infrastructure.migration.AllocationShadowComparator.Divergence;
import com.flowzati.archone.stock.allocation.infrastructure.migration.AllocationShadowComparator.LegacySnapshot;
import com.flowzati.archone.stock.allocation.infrastructure.migration.AllocationShadowComparator.ShadowLine;
...
class AllocationShadowComparatorTest {
  ...
  void shouldClassifyLegacyLocationExpansionAsKnownCorrection() {
    ...
    var comparison = comparator.compare(
        legacy(uuid(99), executionLocation, true),
        demand(executionLocation, true, false, lines()));

    assertThat(comparison.divergences())
        .containsExactly(Divergence.KNOWN_LEGACY_LOCATION_EXPANSION);
  ...
  void shouldUseConfiguredOutboundLocationWithoutExecution() {
    ...
    var comparison = comparator.compare(
        legacy(outboundDefault, null, true),
        demand(outboundDefault, true, false, lines()));
  ...
  void shouldClassifyCrossSkuPrecedenceCorrection() {
    ...
    var comparison = comparator.compare(
        ...
        demand(location, false, true, lines()));

    assertThat(comparison.divergences())
        .containsExactly(Divergence.KNOWN_CROSS_SKU_FIFO_CORRECTION);
    assertThat(comparison.blocksCutover()).isFalse();
  }
  ...
  void shouldBlockUnexplainedDemandAndOutcomeDifferences() {
    ...
    assertThat(comparison.divergences())
        .containsExactlyInAnyOrder(
            Divergence.BLOCKING_DEMAND_CONTENT,
            Divergence.BLOCKING_ALLOCATION_OUTCOME);
    assertThat(comparison.blocksCutover()).isTrue();
  }

  private static LegacySnapshot legacy(
      UUID expandedLocation, UUID executionLocation, boolean eligible) {
    return new LegacySnapshot(
        source(), uuid(1), uuid(2), expandedLocation, executionLocation, uuid(3),
        lines(), eligible);
  }
  ...
  private static DemandSnapshot demand(
      UUID location,
      boolean eligible,
      boolean rejectedOnlyByCrossSku,
      List<ShadowLine> lines) {
    return new DemandSnapshot(
        source(), uuid(1), uuid(2), location, lines, eligible, rejectedOnlyByCrossSku);
  }
