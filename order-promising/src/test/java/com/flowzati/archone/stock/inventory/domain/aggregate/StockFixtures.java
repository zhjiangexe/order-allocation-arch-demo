package com.flowzati.archone.stock.inventory.domain.aggregate;
 ...
 * 批次庫存的測試資料。
 *
 * <p>{@link StockPool} 的建構子有九個參數，其中五個是身分維度。大多數測試只在意數量，把五個
 * 維度逐一寫出來會把測試的重點埋掉。
 *
 * <p><b>對外只開語意化的入口</b>，而不是一個帶五個參數的通用方法——與 {@link OrderFixtures}
...
public final class StockFixtures {
  ...
  public static StockPool unexpiredBatch(
      String skuCode, int onHandQuantity, int reservedQuantity) {
   ...
   */
  public static StockPool batchArrivedOnExpiringOn(
  ...
  public static java.util.Optional<StockPool> reloadUnexpiredBatch(
      com.flowzati.archone.stock.inventory.domain.repository.StockPoolRepository repository,
      String skuCode) {
   ...
   */
  public static StockPool fullyReservedBatch(String skuCode, int quantity) {
