...
class CancelMovementsUsecaseTest {
  ...
  void setUp() {
    ...
    when(operationRepository.save(any())).thenAnswer(invocation -> {
      ...
      return AllocationCancellationOperation.rehydrate(
          operation.allocationDemandId(), operation.operationId(), operation.startedAt(),
          operation.updatedAt(), operation.state(), operation.version());
    });
  ...
  void shouldCancelDemandUnderStableOperationId() {
    ...
    verify(operationRepository).save(org.mockito.ArgumentMatchers.argThat(operation ->
        operation.state() == AllocationCancellationState.COMPLETED));
  }
  ...
  private static AllocationDemand demand(UUID orderId) {
    return AllocationDemand.accept(
        uuid(10), SourceAllocationUnit.primaryOrder(orderId.toString()),
        uuid(11), uuid(12), uuid(13), NOW.plusSeconds(3600), 50, NOW.minusSeconds(60),
