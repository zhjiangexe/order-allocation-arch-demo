package com.flowzati.archone.stock.inventory.entrypoint.rest;

import com.flowzati.archone.bootstrap.time.ConfiguredBusinessClock;
import com.flowzati.archone.stock.inventory.application.usecase.GetStockPoolUsecase;
...
class StockPoolControllerTest {
  ...
  static class FixedClockConfiguration {
    @Bean
    ConfiguredBusinessClock businessCalendar() {
  ...
  void shouldReportEveryBatchInAllocationOrder() {
    givenWarehouseHolds(Map.of("SKU-1", List.of(
    ...
    response.bodyJson().extractingPath("$.skus[0].batches[0].onHandQuantity").isEqualTo(60);
    response.bodyJson().extractingPath("$.skus[0].batches[0].reservedQuantity").isEqualTo(10);
    response.bodyJson().extractingPath("$.skus[0].batches[0].availableToPromise").isEqualTo(50);
  ...
  void shouldReportEverySkuHeldInTheWarehouse() {
    ...
    Map<String, List<StockPool>> held = new LinkedHashMap<>();
  ...
  void shouldMarkExpiredBatchesRatherThanOmitThem() {
    ...
    response.bodyJson().extractingPath("$.skus[0].batches[0].onHandQuantity").isEqualTo(25);
  ...
  void shouldDistinguishAFullyReservedBatchFromAnExpiredOne() {
    ...
    response.bodyJson().extractingPath("$.skus[0].batches[0].availableToPromise").isEqualTo(0);
  }
  ...
  void shouldTreatTheExpiryDateItselfAsNotExpired() {
    givenWarehouseHolds(
        Map.of("SKU-1", List.of(StockFixtures.batchExpiringOn("SKU-1", TODAY, 10, 0))));

    assertThat(mvc.get().uri(QUERY))
        .bodyJson().extractingPath("$.skus[0].batches[0].expired").isEqualTo(false);
  }
