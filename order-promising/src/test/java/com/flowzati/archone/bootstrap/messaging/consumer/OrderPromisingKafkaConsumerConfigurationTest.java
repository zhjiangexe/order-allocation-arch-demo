...
class OrderPromisingKafkaConsumerConfigurationTest {
  ...
  void fixesTheCombinedTransactionAttemptBudget() {
    assertThat(OrderPromisingConsumerFailurePolicy.LOCAL_OPTIMISTIC_RETRIES).isEqualTo(2);
    assertThat(OrderPromisingConsumerFailurePolicy.KAFKA_REDELIVERY_RETRIES).isEqualTo(4);
    assertThat(OrderPromisingConsumerFailurePolicy.MAX_OPTIMISTIC_TRANSACTION_ATTEMPTS)
        .isEqualTo(15);
    assertThat(
        OrderPromisingConsumerFailurePolicy.MAX_TRANSIENT_INFRASTRUCTURE_PROCESSING_ATTEMPTS)
        .isEqualTo(5);
  }
  ...
  private void assertClassification(
      KafkaConsumerFailurePolicy policy,
      Throwable failure,
      MessageFailureClassification expected
  ) {
