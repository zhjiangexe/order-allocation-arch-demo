...
class OrderControllerTest {
    ...
    void shouldPlaceOrderFromJsonBodyAndReturnFullOrderRepresentation() {
        ...
        response.bodyJson().extractingPath("$.facilityId").isEqualTo(OrderFixtures.FACILITY_ID.toString());
        ...
        response.bodyJson().extractingPath("$.dispatchBy").isEqualTo(OrderFixtures.DISPATCH_BY.toString());
        response.bodyJson().extractingPath("$.releasePriority").isEqualTo(OrderFixtures.RELEASE_PRIORITY);
    ...
    void exposesFulfillmentStateAndTimestamp() {
        ...
        Order fulfilled = Order.rehydrate(
                ...
                "EXT-FULFILLED",
                OrderFixtures.deliveryTerms(),
                List.of(OrderLine.create(UUID.randomUUID(), 1, OrderFixtures.OWNER_ID, SKU, 3)),
    ...
    void carriesOwnerIdButNotOwnerName() {
        when(listRecentOrdersUsecase.listRecent(20))
                .thenReturn(List.of(OrderFixtures.pendingOrder(UUID.randomUUID(), SKU, 3, RECEIVED_AT)));

        MvcTestResultAssert response = assertThat(mvc.get().uri("/orders"));

        response.bodyJson().extractingPath("$[0].ownerId").isEqualTo(OrderFixtures.OWNER_ID.toString());
