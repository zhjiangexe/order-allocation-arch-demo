package com.flowzati.archone.inventory.allocation.infrastructure.messaging.consumer.retry;
...
class AllocationOptimisticLockRetryObserverTest {
  ...
  void recordsAllocationRetryAndExhaustionWithBusinessOperationNames() {
    ...
    observer.onExhausted(
        invocation, 3, new OptimisticLockingFailureException("forced conflict"));

    assertThat(meterRegistry.get(AllocationOptimisticLockRetryObserver.RETRY_ATTEMPT_METRIC)
        .tag("operation", "allocate-order").counter().count()).isEqualTo(2.0);
    assertThat(meterRegistry.get(AllocationOptimisticLockRetryObserver.RETRY_EXHAUSTED_METRIC)
        .tag("operation", "allocate-order").counter().count()).isEqualTo(1.0);
  }

  @Test
  void keepsInventoryAvailabilityOperationClassification() {
    observer.onRetry(invocation(
        AllocationEventSubscriptions.INVENTORY_AVAILABILITY,
        StockAvailabilityIncreasedIntegrationEvent.EVENT_TYPE), 2);
  ...
  void ignoresNonAllocationSubscribers() {
    ...
    assertThat(meterRegistry.find(
        AllocationOptimisticLockRetryObserver.RETRY_ATTEMPT_METRIC).counter()).isNull();
    assertThat(meterRegistry.find(
        AllocationOptimisticLockRetryObserver.RETRY_EXHAUSTED_METRIC).counter()).isNull();
  }
