...
class AllocationCancellationTransactionsTest {
    ...
    void shouldCompletePendingCancellationAtomically() {
        ...
        AllocationCancellationStepResult result =
                transactions.completePendingOrRefresh(demand.id(), operation.operationId(), NOW);
    ...
    private void given(AllocationDemand demand, AllocationCancellationOperation operation) {
        when(demandRepository.findById(demand.id())).thenReturn(Optional.of(demand));
        when(operationRepository.find(demand.id(), operation.operationId())).thenReturn(Optional.of(operation));
    }
