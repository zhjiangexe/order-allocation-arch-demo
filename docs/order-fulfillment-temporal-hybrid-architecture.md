...
| 取消子流程 | Shipment 建立前後取消有何差異？WMS 如何回覆最終決策？ | `requestCancellation`、`cancelShipmentInWms`、`cancelOrderInOrdering` |
...
- `execute`、`cancelShipmentInWms`、`cancelOrderInOrdering` 是 orchestration；決定下一個跨系統 checkpoint。
- Signal／Update methods 是外部訊息入口；只接受、correlate 或保存會影響主線的事實與命令。
- `require...`、`ensure...` 與 correlation methods 是 contract guard；不創造新的業務分支，避免錯誤或不相干訊息污染 state。
...
| --- | --- | --- |
| `cancelShipmentInWms` | 要求 WMS 依 Shipment 實際進度同步回覆 `CANCELLED`／`REJECTED` | `CancelShipment` Activity 會 |
| `rejectShipmentCancellation` | 記錄 WMS 拒絕，讓主線回到 handover 等待 | 不會，只更新 replay state |
| `cancelOrderInOrdering` | 在 WMS 已安全後，以同一 request 提交 Ordering cancellation | 其中的 `CancelOrder` Activity 會 |
| `correlatedCarrierHandover` | 對 shipment fact 做 correlation | 不會；不相干 Signal 被安全忽略 |
...
correlated carrier handover、WMS 對取消回覆 `CANCELLED／REJECTED`。其餘方法應能
...
PlaceOrder transaction
  ...
  -> Debezium / Kafka
  -> Temporal starter
  -> WorkflowClient.start(execute, OrderFulfillmentWorkflowInput(orderId, orderReceivedAt))
```
...
| --- | --- | --- | --- |
| `InventoryActivities` | `inventory-context` 的 `TemporalInventoryActivitiesAdapter` | `order-promising-activities` | 要求配貨、完成 outbound movements |
| `OrderingActivities` | `ordering-context` 的 `TemporalOrderingActivitiesAdapter` | `order-promising-activities` | 將 Order 記為 fulfilled／cancelled |
| `WmsActivities` | `wms-context` 的 `TemporalWmsActivitiesAdapter` | `wms-activities` | 冪等建立 Shipment；同步回傳 `CANCELLED`／`REJECTED` 取消決策 |

所有 interfaces 與 transport DTOs 位於 `fulfillment-workflow-contract`；
`OrderFulfillmentWorkflowImpl` 留在 `fulfillment-workflow-runtime`。Activity adapters 由各 bounded
context 擁有，`deployments:monolith` 只負責建立 client／workers 並註冊 adapters。

目前 Ordering 與 Inventory 都部署在 monolith，所以為了保持既有 Workflow history 相容性，
兩個 contracts 仍共用 `order-promising-activities` task queue。未來拆成不同服務時，要透過
...
overdue detector
  ...
  -> Workflow accepts command and resumes at a safe checkpoint
     ...
     -> Shipment exists: CancelShipment Activity
          -> CANCELLED: CancelOrder Activity
          -> REJECTED: do not cancel Order; wait handover and continue fulfillment
...
`PUTBACK_REQUIRED` 與 `REJECTED_AFTER_HANDOVER` 目前都映射為 `REJECTED`，Workflow 不取消 Order，
而是繼續等待 handover。這是因為尚未實作 putback recovery completion；完成該能力前，
不把「需要回庫」誤表示成「已安全取消」。
...
- `OrderFulfillmentWorkflowCancellationState` 是取消流程的唯一控制狀態；nullable request／resolution／timestamp 只表示 payload 尚未產生，並集中由 correlation 或 `require...` invariant 檢查處理。
...
- [x] WMS Activity 直接回傳非 null 的 `CreateShipmentActivityResult`；Workflow 只保存 `shipmentId`。
...
- [x] WMS Activity 同步回 `CANCELLED／REJECTED`；`PUTBACK_REQUIRED` 在 recovery completion 實作前保守映射為 `REJECTED`。
...
- [x] Workflow 測試覆蓋 happy path、長期等待、無 Shipment 取消、建單期間取消，以及 WMS `CANCELLED／REJECTED`。
...
- [x] 將 Workflow／Activity interfaces 抽到 `fulfillment-workflow-contract`，Workflow implementation 留在 runtime。
- [x] 將混合 Inventory／Ordering 的 Activity implementation 拆成 context-owned Temporal adapters。
- [x] `TemporalWmsActivitiesAdapter` 位於 `wms-context`，委派 `CreateShipmentUsecase`、`CancelShipmentUsecase` 並回傳 typed result。
...
- [ ] WMS 內部補上 `PUTBACK_REQUIRED` recovery completion use case；實作後再決定 Activity 是同步等待或新增 final fact。
- [x] Workflow、Inventory／Ordering Activities、WMS Activities 由 monolith 組裝並 poll 對應 task queue。
...
- `cd backend && ./gradlew :fulfillment-workflow-runtime:check`
