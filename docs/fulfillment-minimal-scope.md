...
LocationStock
  (locationId, ownerId, skuCode, expireDate, group) 複合主鍵, quantity, version

Shipment
  id, orderId, ownerId, nodeId, status, createdAt, departedAt

PickTask
  id, shipmentId, orderLineId, locationId, ownerId, skuCode, expireDate, group
...
| `location_stock` | `(location_id, owner_id, sku_code, expire_date, group)` 複合主鍵，含 `version` 樂觀鎖 |
| `shipments` | 出貨單，兩態 |
