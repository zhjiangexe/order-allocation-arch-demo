...
T1～T6 的子任務可連續完成。依使用者最新確認，T7 分成 Events、Temporal、文件收尾，
每階段完成都停下檢查，確認後才進入下一階段。
...
| T7.1 Events | 共用回歸、Events HTTP 與真實 UI 情境證據 |
| T7.2 Temporal | Temporal HTTP 與相同 UI 情境、Workflow 完成及查詢異常證據 |
| T7.3 收尾 | 雙模式完整驗收證據、操作文件與剩餘限制 |
...
新增 /allocations 路由與主選單。唯讀載入 CONFIRMED 作業（固定最多 200 筆），提供貨主、設施、
來源及 SKU 的已載入資料篩選，篩選保留在 URL。作業卡片列出來源、貨主／庫位、排程時間、
識別碼及可展開的逐行需求。支援手動刷新、讀取失敗、舊快照、空集合與達上限提示。
ORDER／PRIMARY 且來源 ID 合法時可開啟共用履約抽屜；補貨需能從主檔解析貨主、來源庫位及 SKU，
...
## T7 分模式驗證安排（2026-09-09 確認）

先完成 Events 並交付檢查，再切換 Temporal 驗證相同業務情境，最後整理文件。
每模式都涵蓋有貨、缺貨補貨、多 SKU 整單分配、FEFO 與跨頁導航，且必須由真實 UI 建單與收貨。
Temporal 另核對 Workflow 啟動、Signal、成功終態與查詢異常處理。
