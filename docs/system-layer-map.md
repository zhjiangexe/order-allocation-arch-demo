...
成立，已排除。`requested_node_id`（貨主指定從哪個倉出）保留，那仍是倉出流程。
...
商務層    Composable Commerce / MACH         目錄、購物車、結帳、金流            ✗ 上游
   ▼
訂單層    DOM                                ① 收單編排 ② Promising ③ Sourcing   ◆ 深做
   ▼
庫存層    IMS                                全網即時庫存視圖                    ◐ 內建
   ▼
...
| 誰需要它 | 需要什麼 |
| --- | --- |
| 訂單層 ③ Sourcing | 分節點的 ATP，作為選節點的決策輸入 |
...
`nodeId` 由 ③ Sourcing 決定，履約層據此知道要在哪個節點揀貨。`ownerId` 決定要揀
哪個貨主的貨——同節點同 SKU 可能有多個貨主的庫存，少了它會揀錯貨。
...
| 節點選擇 | 訂單層 ③ | assignment / transportation | **要做** |
...
**履約層做最小版或深做版，不影響任何演算法的存在與否。** 兩個標註「最小版不做」的
項目都是規則而非演算法。supply-demand 的演算法量體完全由 ② 與 ③ 決定。

## Module 結構

| Module | 內容 | 狀態 |
| --- | --- | --- |
| `order-promising` | `ordering`、`allocation`、`common`、`bootstrap` | 已存在 |
...
即可完整展示**——不需要 ③ 的節點選擇（單節點也有多批次競爭），也不需要 ①A。
...
| ③ Sourcing | 第一步、①A | 節點維度已在第一步完成，③ 只剩節點主檔、覆蓋範圍與選點決策 |
| ①C 放寬多筆 line | 第一步 | `order_lines` 已存在，此段只是放寬「一張單可有多筆」，非搬遷結構 |
| 履約層（最小版） | ③、交會點 2 | 無 `nodeId` 則不知在哪揀貨；無 `ownerId` 與批次則揀錯貨；無 `consume()` 則出庫無法扣帳 |
