...
public class AllocationDemandPlanner {
    ...
    public AllocationDemandPlan plan(AllocationDemand demand, AllocatableBatches batches) {
        ...
        return AllocationDemandPlan.readyToCommit(demand, planPicksByCanonicalLine(canonicalLines, batches));
    }
    ...
    private static void requirePlannable(AllocationDemand demand, AllocatableBatches batches) {
        ...
        batches.requireCovers(
                demand.ownerId(), demand.locationId(), demand.totalsBySku().keySet());
    }
     ...
     * {@code int} 加總時溢位。這一步只讀 {@link StockQuant#availableToPromise()}，不做 reservation。
     */
    private static SkuQuantities availableUpToRequired(SkuQuantities required, AllocatableBatches batches) {
        ...
        for (String skuCode : required.skuCodes()) {
            ...
            for (StockQuant batch : batches.forSku(skuCode)) {
    ...
    private static List<AllocationBatchPick> planPicksByCanonicalLine(
            List<AllocationDemandLine> canonicalLines, AllocatableBatches batches) {
        ...
        // 這裡只回傳試算結果；StockQuant reservation 會在 AllocationCommitter 的 transaction 中執行。
        return picks;
    }
}
