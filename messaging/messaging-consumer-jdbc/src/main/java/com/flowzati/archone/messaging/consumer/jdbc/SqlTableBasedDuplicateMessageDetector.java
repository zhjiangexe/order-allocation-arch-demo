...
public final class SqlTableBasedDuplicateMessageDetector implements DuplicateMessageDetector {
  ...
  private static final List<String> INSERT_COLUMNS = List.of(
      "subscriber_id", "event_id", "event_type", "processed_at");
  ...
  public SqlTableBasedDuplicateMessageDetector(
    ...
    this.claimSql = dialect.insertIgnoringDuplicate(
        schema.qualify(tableNames.inbox()), INSERT_COLUMNS, CONFLICT_COLUMNS);
  }
  ...
  public boolean claimIfNew(String subscriberId, UUID messageId, String messageType) {
    ...
    int affectedRows = statementExecutor.update(claimSql, List.of(
        subscriberId,
        messageId,
        messageType,
    ...
    throw new IllegalStateException(
        "Inbox claim affected an unexpected row count: " + affectedRows);
  }
