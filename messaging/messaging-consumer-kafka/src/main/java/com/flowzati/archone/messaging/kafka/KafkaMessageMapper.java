...
public final class KafkaMessageMapper {
  ...
  public KafkaMessageMapper(MessageHeadersDecoder headersDecoder) {
    this.headersDecoder = Objects.requireNonNull(
        headersDecoder, "Message headers decoder is required");
  }
  ...
  private void validateNoPhysicalCollision(
    ...
    if (timestamp >= 0) {
      requireAbsentOrEqual(
          headers, MessageHeaders.MESSAGE_DATE, Instant.ofEpochMilli(timestamp).toString());
    } else {
  ...
  private void requireAbsentOrEqual(
      Map<String, String> headers,
      String name,
