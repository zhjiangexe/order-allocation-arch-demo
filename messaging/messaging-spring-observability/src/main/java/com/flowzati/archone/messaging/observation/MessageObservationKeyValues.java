...
final class MessageObservationKeyValues {
  ...
  static KeyValues traceOnlyIdentifiers(Message message) {
    ...
    keyValues = addHeader(
        keyValues, message, MessageHeaders.MESSAGE_ID, MessagingObservationTags.MESSAGE_ID);
    keyValues = addHeader(
        keyValues, message, MessageHeaders.PARTITION_ID, MessagingObservationTags.PARTITION_ID);
    return addHeader(
        keyValues, message, MessageHeaders.CORRELATION_ID,
        MessagingObservationTags.CORRELATION_ID);
  }

  private static KeyValues addHeader(
      KeyValues keyValues,
      Message message,
      String header,
