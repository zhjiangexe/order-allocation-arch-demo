...
public final class MessageHandlerDecoratorChain {
  ...
  private MessageHandlerDecoratorChain(
      ...
      OutcomeMessageHandler terminalHandler,
      int index
  ) {
    ...
    for (MessageHandlerDecorator decorator : ordered) {
      if (!seenOrders.add(decorator.order())) {
        throw new IllegalArgumentException(
            "Duplicate message handler decorator order: " + decorator.order());
      }
    }
  ...
  public ProcessingOutcome invokeNext(MessageHandlerInvocation invocation) {
    ...
    MessageHandlerDecoratorChain remainder = new MessageHandlerDecoratorChain(
        decorators, terminalHandler, index + 1);
    ProcessingOutcome outcome = decorator.handle(invocation, remainder);
    return Objects.requireNonNull(outcome, () ->
        "Message handler decorator returned null: " + decorator.getClass().getName());
  }
}
