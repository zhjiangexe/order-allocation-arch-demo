...
public final class MessageInterceptorDecorator implements MessageHandlerDecorator {
    ...
    private Throwable invokePostHooks(
        ...
        for (int index = handling.size() - 1; index >= 0; index--) {
            firstFailure = invokePostHandle(handling.get(index), message, context, handlingFailure, firstFailure);
        }
        for (int index = received.size() - 1; index >= 0; index--) {
            firstFailure = invokePostReceive(received.get(index), message, context, handlingFailure, firstFailure);
        }
        return firstFailure;
    }
