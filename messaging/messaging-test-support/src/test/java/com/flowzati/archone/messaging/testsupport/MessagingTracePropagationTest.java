...
class MessagingTracePropagationTest {
  ...
  void persistsW3cHeadersAndContinuesTheTraceAcrossOutboxAndConsumer() {
    try (TracingFixture tracing = tracing()) {
      ...
      assertThat(persisted.requiredHeader(MessageHeaders.TRACESTATE))
  ...
  private TracingFixture tracing() {
    ...
    io.opentelemetry.api.trace.Tracer otelTracer =
        openTelemetry.getTracer("archone-messaging-test");
    ...
    OtelPropagator propagator = new OtelPropagator(
        openTelemetry.getPropagators(), otelTracer);
  ...
  private static final class TracingFixture implements AutoCloseable {
    ...
    private Message publish(Message message) {
      ...
      MessagePublicationContext publication =
    ...
    private String consume(Message message) {
      ...
      chain.invokeNext(new MessageHandlerInvocation(
          message, new MessageContext("ordering", "order-events", 1)));
      return traceId.get();
    }
