  ...
  public SpringMessagingTransactionTemplate(PlatformTransactionManager transactionManager) {
    this(
        new TransactionTemplate(Objects.requireNonNull(
            transactionManager, "PlatformTransactionManager is required")),
        TransactionSynchronizationManager::isActualTransactionActive);
  }

  SpringMessagingTransactionTemplate(
      TransactionOperations transactionOperations,
      BooleanSupplier activeTransaction
  ) {
    this.transactionOperations = Objects.requireNonNull(
        transactionOperations, "TransactionOperations is required");
    this.activeTransaction = Objects.requireNonNull(
        activeTransaction, "Active transaction probe is required");
  }
  ...
  public void requireActive() {
    if (!isActive()) {
      throw new IllegalStateException(
          "No active caller transaction for transactional message production");
    }
  }
