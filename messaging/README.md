...
每個 bounded context 各自建立一個小型 subscriber configuration：明確宣告 handlers，並由
factory 建立唯一的 subscription owner。這是刻意保留的 Tram-shaped application surface，不採用
handler scan、全域 handler catalog 或 application `@KafkaListener`：

```java
@Configuration(proxyBeanMethods = false)
@ConditionalOnIntegrationEventConsumption
class AllocationOrderLifecycleEventConfiguration {

  @Bean
  IntegrationEventHandlers allocationOrderLifecycleHandlers(Target target) {
...
`subscriberId` 是 Inbox idempotency scope；`consumerGroupId` 是 Kafka delivery identity。兩者
可以同值，但不可視為同一概念。`IntegrationEventNameMapping` 必須顯式列出允許反序列化的 stable
event type／version，不使用 Java FQCN 動態載入。`@ConditionalOnIntegrationEventConsumption` 是單一
...
| `subscriberId`、`consumerGroupId`、unhandled policy | concurrency／ack／shutdown 的共用預設 |
