...
public final class KafkaDeadLetterErrorHandlerFactory {
    ...
    public static KafkaSubscriptionErrorHandlerFactory perSubscription(
        ...
        return subscription -> {
            ...
            KafkaConsumerFailurePolicy policy = Objects.requireNonNull(
                    failurePolicyResolver.resolve(subscription), "Kafka failure policy resolver returned null");
            KafkaConsumerFailureObserver observer = Objects.requireNonNull(
                    observerFactory.apply(subscription), "Kafka failure observer factory returned null");
    ...
    public static DefaultErrorHandler create(
        ...
        return create(
                kafkaOperations, retryBackOff, failureClassifier, headersProvider, KafkaConsumerFailureObserver.none());
    }
    ...
    public static DefaultErrorHandler create(
        ...
        recoverer.setHeadersFunction((record, failure) -> Objects.requireNonNull(
                headersProvider.headersFor(record, failure), "Kafka dead-letter headers provider returned null"));

        KafkaRetryObservationHooks observationHooks =
                new KafkaRetryObservationHooks(failureObserver, failureClassifier);
        DefaultErrorHandler errorHandler =
                new DefaultErrorHandler(recoverer, retryBackOff, observationHooks.observingBackOffHandler());
        ...
        errorHandler.setBackOffFunction(
                (record, failure) -> failureClassifier.classify(failure).retryable() ? retryBackOff : NO_RETRY);
        return errorHandler;
    }
}
