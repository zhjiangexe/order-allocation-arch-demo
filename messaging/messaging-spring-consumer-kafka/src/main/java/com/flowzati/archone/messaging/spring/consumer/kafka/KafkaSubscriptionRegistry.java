...
final class KafkaSubscriptionRegistry {
  ...
  synchronized void reserve(KafkaSubscriptionRegistration candidate) {
    ...
    if (duplicate != null) {
      throw new IllegalStateException(
          "Duplicate Kafka subscriber ID: " + candidate.subscriberId());
    }
  ...
  private void validateNoCollision(
      KafkaSubscriptionRegistration existing,
      KafkaSubscriptionRegistration candidate
  ) {
    if (existing.containerId().equals(candidate.containerId())) {
      throw new IllegalStateException(
          "Duplicate Kafka container ID: " + candidate.containerId());
    }
