...
public final class KafkaDeadLetterReplayRecordFactory {
  ...
  public <K, V> ProducerRecord<K, V> create(ConsumerRecord<K, V> deadLetterRecord) {
    ...
    String originalTopic = requiredTextHeader(
        deadLetterRecord.headers(), KafkaHeaders.DLT_ORIGINAL_TOPIC);
    String physicalDestination = requiredTextHeader(
        deadLetterRecord.headers(), KafkaDeadLetterHeaders.ORIGINAL_PHYSICAL_DESTINATION);
    requiredTextHeader(
        deadLetterRecord.headers(), KafkaDeadLetterHeaders.ORIGINAL_LOGICAL_CHANNEL);
    ...
    if (!originalTopic.equals(physicalDestination)) {
      throw new IllegalArgumentException(
          "DLT original topic does not match physical destination");
    }

    int originalPartition = requiredIntHeader(
        deadLetterRecord.headers(), KafkaHeaders.DLT_ORIGINAL_PARTITION);
    requiredLongHeader(deadLetterRecord.headers(), KafkaHeaders.DLT_ORIGINAL_OFFSET);
    long originalTimestamp = requiredLongHeader(
        deadLetterRecord.headers(), KafkaHeaders.DLT_ORIGINAL_TIMESTAMP);
