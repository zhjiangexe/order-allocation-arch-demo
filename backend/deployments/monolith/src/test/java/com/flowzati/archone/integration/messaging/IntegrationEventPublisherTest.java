...
class IntegrationEventPublisherTest {
    ...
    private static final UUID FACILITY_ID = UUID.fromString("00000000-0000-0000-0000-0000000000b1");
    private static final List<LineSnapshot> LINES = List.of(new LineSnapshot(1, "SKU-1", 3));
    ...
    void shouldUseTheStockContentionKeyForPlacedAndCancelled() {
        RecordingPublisher publisher = new RecordingPublisher();
        OrderingIntegrationEventPublisher publisherAdapter = new OrderingIntegrationEventPublisher(publisher, "stock");
        UUID orderId = UUID.randomUUID();
        ...
        assertThat(publisher.publications)
                ...
                .containsExactly(OWNER_ID + "/" + FACILITY_ID, OWNER_ID + "/" + FACILITY_ID);
        assertThat(publisher.publications)
        ...
        assertThat(publisher.publications)
                ...
                .containsExactly(OrderPlacedIntegrationEvent.EVENT_TYPE, OrderCancelledIntegrationEvent.EVENT_TYPE);
    }

    @Test
    @DisplayName("兩種策略都支援多 SKU 訂單，且 partition key 不含 SKU")
    void shouldTranslateMultiSkuOrdersUnderEveryStrategy() {
        List<LineSnapshot> twoSkus = List.of(new LineSnapshot(1, "SKU-1", 3), new LineSnapshot(2, "SKU-2", 5));

        for (String strategy : List.of("order-id", "stock")) {
            RecordingPublisher publisher = new RecordingPublisher();
            UUID orderId = UUID.randomUUID();

            new OrderingIntegrationEventPublisher(publisher, strategy)
                    .publish(new OrderPlaced(
                            orderId, OWNER_ID, FACILITY_ID, "100", LocalDate.of(2026, 8, 1), twoSkus, OCCURRED_AT));
    ...
    void shouldTranslateAllocationOutcomes() {
        RecordingPublisher publisher = new RecordingPublisher();
        AllocationIntegrationEventPublisher publisherAdapter = new AllocationIntegrationEventPublisher(publisher);
        UUID orderId = UUID.randomUUID();
        ...
        publisherAdapter.publish(new OrderAllocationCompleted(
                ...
                OWNER_ID,
                FACILITY_ID,
                List.of(new OrderAllocationCompleted.AllocationLine(orderLineId, moveId, "SKU-1", locationId, 3)),
        ...
        assertThat(publisher.publications.get(0).aggregate().type()).isEqualTo(OrderingAggregateTypes.ORDER);
        ...
        assertThat(fulfillmentPublication.aggregate().type()).isEqualTo(FulfillmentAggregateTypes.STOCK_PICKING);
        assertThat(fulfillmentPublication.aggregate().id()).isEqualTo(allocationId.toString());
        assertThat(fulfillmentPublication.target().destination()).isEqualTo(FulfillmentChannels.FULFILLMENT_HANDOFFS);
        var fulfillment = (AllocationCommittedForFulfillmentIntegrationEvent) fulfillmentPublication.event();
        ...
        assertThat(publisher.publications).allSatisfy(publication -> {
            assertThat(publication.target().partitionKey()).isEqualTo(orderId.toString());
            assertThat(publication.occurredAt()).isEqualTo(OCCURRED_AT);
        });
    }
    ...
    void shouldTranslateStockAvailabilityIncrease() {
        ...
        new InventoryIntegrationEventPublisher(publisher)
                .publish(new StockAvailabilityIncreased(OWNER_ID, FACILITY_ID, locationId, "SKU-1", 12, OCCURRED_AT));

        IntegrationEventPublication publication = publisher.onlyPublication();
        assertThat(publication.event()).isInstanceOf(StockAvailabilityIncreasedIntegrationEvent.class);
    ...
    void shouldSeparateStockKeysForDifferentOwners() {
        ...
        publisherAdapter.publish(new OrderPlaced(
                UUID.randomUUID(), otherOwnerId, FACILITY_ID, "100", LocalDate.of(2026, 8, 1), LINES, OCCURRED_AT));
    ...
    private static OrderPlaced placed(UUID orderId) {
        return new OrderPlaced(orderId, OWNER_ID, FACILITY_ID, "100", LocalDate.of(2026, 8, 1), LINES, OCCURRED_AT);
    }
