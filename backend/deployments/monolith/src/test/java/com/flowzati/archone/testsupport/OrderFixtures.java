...
public final class OrderFixtures {
    ...
    public static final UUID OTHER_LOCATION_ID = UUID.fromString("00000000-0000-0000-0000-0000000000c2");

    public static final UUID OTHER_OWNER_ID = UUID.fromString("00000000-0000-0000-0000-0000000000a2");
    ...
    public static void seedCatalog(JdbcTemplate jdbcTemplate, UUID ownerId, String... skuCodes) {
        ...
        seedInboundType(jdbcTemplate, MovementFixtures.INBOUND_TYPE_ID, FACILITY_ID, LOCATION_ID, "測試倉收貨");
        seedInboundType(
                jdbcTemplate, MovementFixtures.OTHER_INBOUND_TYPE_ID, OTHER_FACILITY_ID, OTHER_LOCATION_ID, "第二個倉收貨");
        jdbcTemplate.update("""
     ...
     */
    public static final UUID OTHER_FACILITY_ID = UUID.fromString("00000000-0000-0000-0000-0000000000bf");

    public static DeliveryTerms deliveryTerms(UUID facilityId) {
        return new DeliveryTerms(
                facilityId, "100", "台北市中正區重慶南路一段 122 號", LocalDate.of(2026, 8, 1), DISPATCH_BY, RELEASE_PRIORITY);
    }
    ...
    public static Order pendingOrder(UUID orderId, UUID ownerId, String skuCode, int quantity, Instant receivedAt) {
        return order(orderId, ownerId, skuCode, quantity, OrderStatus.PENDING, receivedAt, null, null, null);
    }
    ...
    public static Order backorderedOrder(
            ...
            Instant backorderedAt,
            Long version) {
        return order(orderId, ownerId, skuCode, quantity, OrderStatus.PENDING, receivedAt, null, null, version);
    }
     ...
     * 配），但斷言常常照行號寫。
     */
    public static Order pendingMultiSkuOrder(UUID orderId, Instant receivedAt, Map<String, Integer> quantitiesBySku) {
        List<OrderLine> lines = new ArrayList<>();
        quantitiesBySku.forEach((skuCode, quantity) ->
                lines.add(OrderLine.create(IdGenerator.nextId(), lines.size() + 1, OWNER_ID, skuCode, quantity)));
        return Order.rehydrate(
                orderId,
                OWNER_ID,
                "EXT-" + orderId,
    ...
    public static Order allocatedOrder(
        ...
        return order(orderId, OWNER_ID, skuCode, quantity, OrderStatus.ALLOCATED, receivedAt, allocatedAt, null, null);
    }
