...
public record AllocationCommitted(
    ...
    public AllocationCommitted {
        ...
        if (moves.stream()
                .anyMatch(move -> !allocationDemandId.equals(move.allocationDemandId())
        ...
        public CommittedAllocationMove {
            ...
            try {
                committedQuantity = batchPicks.stream()
                        .mapToInt(CommittedBatchPick::quantity)
                        .reduce(0, Math::addExact);
            } catch (ArithmeticException overflow) {
                throw new IllegalArgumentException("Committed batch-pick quantity exceeds integer range", overflow);
            }
    ...
    public record CommittedBatchPick(UUID stockQuantId, int quantity) {
        public CommittedBatchPick {
            if (stockQuantId == null || quantity <= 0) {
                throw new IllegalArgumentException("Committed batch pick requires stock quant and quantity");
            }
        }
    }
}
