...
import com.flowzati.archone.inventory.position.infrastructure.messaging.StockAvailabilityPublisherAdapter;
...
class StockAvailabilityPublisherAdapterTest {
    ...
    void mapsApplicationAvailabilityFactToExistingV1ContractAndContentionRouting() {
        ...
        var adapter = new StockAvailabilityPublisherAdapter(integrationEventPublisher);
        Instant occurredAt = Instant.parse("2026-08-27T04:00:00Z");
        StockAvailabilityIncrease increase =
                new StockAvailabilityIncrease(uuid(1), uuid(2), uuid(3), "SKU-A", 10, occurredAt);
