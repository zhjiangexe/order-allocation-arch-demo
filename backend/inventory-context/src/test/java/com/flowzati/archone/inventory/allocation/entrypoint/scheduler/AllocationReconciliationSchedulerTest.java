...
class AllocationReconciliationSchedulerTest {
    ...
    void shouldConditionTheSchedulerBeanInsteadOfTheSchedulingEngine() {
        ...
        runner.withPropertyValues("archone.allocation.reconciliation-scheduler-enabled=false")
                .run(context -> assertThat(context).doesNotHaveBean(AllocationReconciliationScheduler.class));
        runner.withPropertyValues("archone.allocation.reconciliation-scheduler-enabled=true")
                .run(context -> assertThat(context).hasSingleBean(AllocationReconciliationScheduler.class));
    }
    ...
    void shouldReconcileWaitingScopesThroughTheSharedUsecase() {
        ...
        verify(allocationAttempt)
                .attempt(new AllocateWaitingDemandCommand(
    ...
    void shouldDeferConflictedScopeUntilTheNextRun() {
        ...
        WaitingAllocationScope following = new WaitingAllocationScope(
                InventoryFixtures.OWNER_ID, InventoryFixtures.FACILITY_ID, InventoryFixtures.LOCATION_ID, "SKU-2");
        ...
        AllocateWaitingDemandCommand followingCommand = new AllocateWaitingDemandCommand(
                InventoryFixtures.OWNER_ID, InventoryFixtures.FACILITY_ID, InventoryFixtures.LOCATION_ID, "SKU-2");
        when(demands.findAllocatablePendingScopes(TODAY, 25)).thenReturn(List.of(conflicted, following));
        doThrow(new OptimisticLockingFailureException("conflict"))
                .when(allocationAttempt)
                .attempt(conflictedCommand);
    ...
    private ReconcileWaitingDemandUsecase reconcileUsecase(
            AllocationDemandRepository demands, TransactionalAllocationAttempt allocationAttempt) {
