 ...
 */
public final class OrderFulfillmentProcessWorkflowImpl implements OrderFulfillmentProcessWorkflow {

    private static final String WMS_SHIPMENT_FACT_CONFLICT = "WMS_SHIPMENT_FACT_CONFLICT";
    private static final String WORKFLOW_INVARIANT_VIOLATION = "ORDER_FULFILLMENT_WORKFLOW_INVARIANT_VIOLATION";
    ...
    private final OrderPromisingActivities orderPromisingActivities = Workflow.newActivityStub(
            OrderPromisingActivities.class, activityOptions(OrderPromisingActivities.TASK_QUEUE));
    ...
    public CancellationRequestAck requestCancellation(CancellationRequest request) {
        ...
        if (cancellation.state == CancellationState.REJECTED || hasCorrelatedCarrierHandover()) {
    ...
    private CancellationRequest requireCancellationRequest() {
        if (cancellation.request == null) {
            throw workflowInvariantViolation(
    ...
    private Result finish(Outcome outcome, String detail) {
        progress = new Progress(Phase.FINISHED, outcome, workflowNow(), detail);
        return new Result(
                input.orderId(),
                outcome,
                allocationCheckpoint.allocationId(),
    ...
    private record Progress(Phase phase, Outcome outcome, Instant updatedAt, String detail) {}
}
