...
public final class EventMessageHeaders {
    ...
    public static int contractVersion(Message message) {
        String value = message.header(EVENT_CONTRACT_VERSION).orElse(Integer.toString(INITIAL_CONTRACT_VERSION));
        try {
            ...
            throw new IntegrationEventContractException("Invalid event-contract-version header: " + value, exception);
        }
    }
