...
public final class SpringKafkaMessageConsumerImplementation implements MessageConsumerImplementation, AutoCloseable {

    private static final System.Logger LOGGER =
            System.getLogger(SpringKafkaMessageConsumerImplementation.class.getName());
    ...
    private final Map<String, DefaultKafkaMessageSubscription> activeSubscriptions = new LinkedHashMap<>();
    ...
    public SpringKafkaMessageConsumerImplementation(
        ...
        this(containerFactory, messageMapper, policyResolver, KafkaSubscriptionErrorHandlerFactory.none());
    }
    ...
    SpringKafkaMessageConsumerImplementation(
        ...
        this.containerFactory =
                Objects.requireNonNull(containerFactory, "Kafka listener container factory is required");
        this.messageMapper = Objects.requireNonNull(messageMapper, "Kafka message mapper is required");
        this.policyResolver = Objects.requireNonNull(policyResolver, "Kafka subscription policy resolver is required");
        this.errorHandlerFactory =
                Objects.requireNonNull(errorHandlerFactory, "Kafka subscription error handler factory is required");
        this.registry = Objects.requireNonNull(registry, "Kafka subscription registry is required");
        this.containerStarter = Objects.requireNonNull(containerStarter, "Kafka container starter is required");
    }

    @Override
    public synchronized MessageSubscription subscribe(
            ResolvedMessageSubscription subscription, MessageHandler handler) {
        ...
        String containerId = KafkaContainerIdentity.from(subscription.subscriberId(), subscription.consumerGroupId());
        ...
        DefaultKafkaMessageSubscription handle =
                new DefaultKafkaMessageSubscription(registration, container, () -> release(registration));
    ...
    private boolean configure(
        ...
        policy.missingTopicsFatalOverride().ifPresent(container.getContainerProperties()::setMissingTopicsFatal);
        policy.shutdownTimeoutOverride()
                .ifPresent(timeout -> container.getContainerProperties().setShutdownTimeout(timeout.toMillis()));
        policy.observationEnabledOverride().ifPresent(container.getContainerProperties()::setObservationEnabled);
        ...
        container.getContainerProperties().setMessageListener((MessageListener<String, String>)
                record -> dispatch(record, subscription, handler));
        LOGGER.log(
    ...
    private void dispatch(
            ConsumerRecord<String, String> record, ResolvedMessageSubscription subscription, MessageHandler handler) {
    ...
    private MappedInboundMessage mapInboundMessage(
            ConsumerRecord<String, String> record, ResolvedMessageSubscription subscription) {
        try {
            ...
            throw new MessageMappingException("Cannot map Kafka record to the generic message envelope", exception);
        }
    }
    ...
    private void cleanupAfterStartFailure(DefaultKafkaMessageSubscription handle, Throwable startFailure) {
        try {
            handle.stop();
    ...
    private record MappedInboundMessage(Message message, MessageContext context) {}
}
