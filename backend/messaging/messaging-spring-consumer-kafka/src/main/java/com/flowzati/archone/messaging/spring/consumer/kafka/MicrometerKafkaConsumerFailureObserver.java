...
public final class MicrometerKafkaConsumerFailureObserver implements KafkaConsumerFailureObserver {
    ...
    public MicrometerKafkaConsumerFailureObserver(
            ObservationRegistry observationRegistry, KafkaConsumerObservationMetadataResolver metadataResolver) {
        this.observationRegistry = Objects.requireNonNull(observationRegistry, "Observation registry is required");
        this.metadataResolver =
                Objects.requireNonNull(metadataResolver, "Kafka observation metadata resolver is required");
    }
    ...
    public void retryScheduled(KafkaConsumerFailureContext context, long nextBackOffMillis) {
        Observation observation = observation(
                ...
                .highCardinalityKeyValue(
                        MessagingObservationTags.RETRY_BACKOFF_MILLIS, Long.toString(nextBackOffMillis));
    ...
    private Observation observation(
        ...
        Observation observation = Observation.createNotStarted(name, observationRegistry)
                ...
                .lowCardinalityKeyValue(MessagingObservationTags.SUBSCRIBER_ID, metadata.subscriberId())
                .lowCardinalityKeyValue(MessagingObservationTags.LOGICAL_DESTINATION, metadata.logicalChannel())
                ...
                .lowCardinalityKeyValue(MessagingObservationTags.EXCEPTION_TYPE, exceptionType(observedFailure))
                .lowCardinalityKeyValue(
                ...
                .highCardinalityKeyValue(MessagingObservationTags.KAFKA_PARTITION, Integer.toString(record.partition()))
                .highCardinalityKeyValue(MessagingObservationTags.KAFKA_OFFSET, Long.toString(record.offset()));
        if (record.key() != null) {
            observation.highCardinalityKeyValue(
                    MessagingObservationTags.PARTITION_ID, record.key().toString());
        }
        textHeader(record, KafkaMessageMapper.LEGACY_ID_HEADER)
                .ifPresent(messageId ->
