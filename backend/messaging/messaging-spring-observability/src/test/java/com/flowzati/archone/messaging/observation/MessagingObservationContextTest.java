...
class MessagingObservationContextTest {
    ...
    void consumerContextExtractsPropagationFieldsCaseInsensitively() {
        ...
        ConsumerMessageObservationContext context =
                new ConsumerMessageObservationContext(traced, new MessageContext("ordering", "order-events", 2));

        assertThat(context.getGetter().get(context.getCarrier(), "TraceParent")).isEqualTo(TRACEPARENT);
    }
