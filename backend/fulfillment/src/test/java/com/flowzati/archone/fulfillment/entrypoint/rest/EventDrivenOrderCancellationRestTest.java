...
@WebMvcTest(EventDrivenOrderCancellationRest.class)
@Import(GlobalRestExceptionHandler.class)
@TestPropertySource(
        properties = {
            "archone.fulfillment.orchestration-mode=events",
...
class EventDrivenOrderCancellationRestTest {
    ...
    void shouldAcceptCancellationRequest() {
        ...
        verify(usecase)
                .accept(new FulfillmentCancellationCommand(
    ...
    void shouldReportRejectedOrConflictingCancellationAsHttp409(FulfillmentCancellationStatus status) {
        when(usecase.accept(any())).thenReturn(new FulfillmentCancellationResult(status, REQUEST_ID));
