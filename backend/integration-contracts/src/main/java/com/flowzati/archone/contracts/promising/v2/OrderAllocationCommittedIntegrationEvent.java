package com.flowzati.archone.contracts.promising.v2;
...
/** Move-centric order assignment fact. V1 remains readable during the compatibility window. */
public final class OrderAllocationCommittedIntegrationEvent extends IntegrationEvent {
    ...
    public OrderAllocationCommittedIntegrationEvent(
            ...
            @JsonProperty("pickingTypeId") UUID pickingTypeId,
        ...
        if (pickingId == null
                ...
                || moves == null
                || moves.isEmpty()) {
            throw new IllegalArgumentException("Move-centric order assignment requires operation, route and moves");
        }
        ...
        if (moves.stream().anyMatch(move -> move == null || !moveIds.add(move.moveId()))) {
            throw new IllegalArgumentException("Move-centric order assignment requires unique non-null moves");
        }
