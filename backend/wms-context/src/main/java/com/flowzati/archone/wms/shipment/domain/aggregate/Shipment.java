...
public class Shipment {
    ...
    public void releaseToWave(UUID waveId, Instant releasedAt) {
        ...
        if (waveId == null) {
            throw new IllegalArgumentException("Wave release requires a Wave ID");
        }
    ...
    public void recordPickingStarted(Instant occurredAt) {
        requireTime(occurredAt, "Picking status time is required");
        if (status == ShipmentStatus.PICKING) {
            return;
        }
        requireStatus(ShipmentStatus.RELEASED, "Only a released Shipment can start picking");
        status = ShipmentStatus.PICKING;
    }

    public void recordPickingCompleted(Instant occurredAt) {
        requireTime(occurredAt, "Picking completion time is required");
        if (status == ShipmentStatus.PICKED) {
            return;
        }
        if (status != ShipmentStatus.RELEASED && status != ShipmentStatus.PICKING) {
            throw new IllegalStateException("Only a released or picking Shipment can complete picking, was " + status);
        }
    ...
    public java.util.Optional<ShipmentCancellationState> cancellationStateValue() {
        return java.util.Optional.ofNullable(cancellationState);
    }
