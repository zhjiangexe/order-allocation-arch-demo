...
public class WmsInboundOperationEntity {
    ...
    private List<WmsInboundExpectedLineEntity> expectedLines = new ArrayList<>();

    @Version
    @Column(nullable = false)
    private Long version;

    protected WmsInboundOperationEntity() {}

    public WmsInboundOperationEntity(InboundOperation operation) {
    ...
    public void replaceFrom(InboundOperation operation) {
        ...
        operation.expectedLines().stream()
                .map(WmsInboundExpectedLineEntity::new)
                .forEach(this.expectedLines::add);
    }
    ...
    public InboundOperation toDomain() {
        return InboundOperation.rehydrate(
                ...
                expectedLines.stream()
                        .map(WmsInboundExpectedLineEntity::toDomain)
                        .toList(),
                status);
    }
}
