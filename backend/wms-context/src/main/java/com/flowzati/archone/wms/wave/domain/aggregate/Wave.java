package com.flowzati.archone.wms.wave.domain.aggregate;
...
public class Wave {
    ...
    public static Wave rehydrate(
            ...
            Instant plannedAt,
            WaveStatus status,
            int pickingWorkCount,
            int pickTaskCount) {
        Wave wave = new Wave(id, facilityId, templateCode, policy, assignments, plannedAt);
        if (status == null || pickingWorkCount < 0 || pickTaskCount < 0) {
            throw new IllegalArgumentException("Persisted Wave state is invalid");
        }
        wave.status = status;
        wave.pickingWorkCount = pickingWorkCount;
        wave.pickTaskCount = pickTaskCount;
        return wave;
    }

    public void release(int pickingWorkCount, int pickTaskCount, Instant releasedAt) {
        requireTime(releasedAt, "Wave release time is required");
        if (status == WaveStatus.RELEASED || status == WaveStatus.COMPLETED) {
            return;
        }
        if (pickingWorkCount < 0
                || pickingWorkCount > assignments.size()
                || pickTaskCount < pickingWorkCount
                || (pickingWorkCount == 0 && pickTaskCount != 0)) {
