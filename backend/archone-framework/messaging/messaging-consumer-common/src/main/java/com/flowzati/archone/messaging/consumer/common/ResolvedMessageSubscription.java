...
public record ResolvedMessageSubscription(
        String subscriberId, String consumerGroupId, Map<String, String> destinationToLogicalChannel) {

    public ResolvedMessageSubscription {
        if (isBlank(subscriberId)
                || isBlank(consumerGroupId)
                || destinationToLogicalChannel == null
                || destinationToLogicalChannel.isEmpty()) {
