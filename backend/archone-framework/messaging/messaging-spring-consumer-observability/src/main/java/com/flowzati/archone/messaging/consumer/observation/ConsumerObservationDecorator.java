...
public final class ConsumerObservationDecorator implements MessageHandlerDecorator {
    ...
    public MessageProcessingStatus handle(MessageHandlerInvocation invocation, MessageHandlerDecoratorChain chain) {
        ...
        try (Observation.Scope ignored = observation.openScope()) {
            ...
            observationContext.recordOutcome(MessagingObservationStatus.FAILED);
    ...
    private MessagingObservationStatus toObservationOutcome(MessageProcessingStatus outcome) {
        return switch (outcome) {
            case PROCESSED -> MessagingObservationStatus.PROCESSED;
            case DUPLICATE -> MessagingObservationStatus.DUPLICATE;
            case IGNORED_UNHANDLED -> MessagingObservationStatus.IGNORED_UNHANDLED;
        };
    }
}
