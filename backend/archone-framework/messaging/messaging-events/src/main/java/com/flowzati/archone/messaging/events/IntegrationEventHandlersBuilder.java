...
public final class IntegrationEventHandlersBuilder {
    ...
    public <E extends IntegrationEvent> IntegrationEventHandlersBuilder onEvent(
            Class<E> eventClass, Consumer<IntegrationEventEnvelope<E>> handler) {
        ...
        if (!registeredKeys.add(key)) {
            throw new IllegalStateException(
                    "Duplicate Integration Event handler: " + destination + "/" + eventClass.getName());
        }
    ...
    private record HandlerKey(String destination, Class<? extends IntegrationEvent> eventClass) {}
}
