package com.flowzati.archone.wms.shipment.application.store;
...
/** Shipment capability 的 persistence port。 */
public interface ShipmentStore {

    Optional<Shipment> findById(UUID shipmentId);

    Optional<Shipment> findByStockOperationId(UUID stockOperationId);
     ...
     * scheduler 掃描時提早載入 Shipment lines；真正處理時會在新的 transaction 重新讀取
     * aggregate 並再次檢查狀態。
     */
