...
class OrderingShipmentCancellationEventConsumerTest {
    ...
    void cancelsOrderAtTheActualShipmentCancellationTime() {
        ...
        when(cancelOrderUsecase.cancel(new CancelOrderCommand(
                        event.getCancellationRequestId(),
                        event.getOrderId(),
                        event.getCancelledAt(),
                        event.getCancellationReason())))
                .thenReturn(Order.CancellationStatus.CANCELLED);
    ...
    @Test
    void exposesAnInvariantFailureWhenOrderingRejectsAfterWmsCancellation() {
        ShipmentCancelledIntegrationEvent event = event();
        when(cancelOrderUsecase.cancel(new CancelOrderCommand(
                        event.getCancellationRequestId(),
                        event.getOrderId(),
                        event.getCancelledAt(),
                        event.getCancellationReason())))
                .thenReturn(Order.CancellationStatus.REJECTED);

        assertThatThrownBy(() -> consumer.onShipmentCancelled(event))
                .isInstanceOf(IllegalStateException.class)
                .hasMessageContaining("rejected cancellation");
    }
