...
class OrderRestTest {
    ...
    void shouldPlaceOrderFromJsonBodyAndReturnFullOrderRepresentation() {
        ...
        when(placeOrderUsecase.placeOrder(any(PlaceOrderCommand.class)))
                .thenReturn(OrderingFixtures.pendingOrder(orderId, SKU, 3, RECEIVED_AT));
        ...
        response.bodyJson().extractingPath("$.ownerId").isEqualTo(OrderingFixtures.OWNER_ID.toString());
        response.bodyJson().extractingPath("$.facilityId").isEqualTo(OrderingFixtures.FACILITY_ID.toString());
        ...
        response.bodyJson().extractingPath("$.dispatchBy").isEqualTo(OrderingFixtures.DISPATCH_BY.toString());
        response.bodyJson().extractingPath("$.releasePriority").isEqualTo(OrderingFixtures.RELEASE_PRIORITY);
    ...
    void rejectsCommandsTheAggregateRefuses() {
        when(placeOrderUsecase.placeOrder(any(PlaceOrderCommand.class)))
                .thenThrow(new IllegalArgumentException("Order must contain at least one line"));
    ...
    void rejectsOrdersWithoutReleasePriority() {
        assertThat(mvc.post()
                        .uri("/orders")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(BODY_WITHOUT_RELEASE_PRIORITY))
                .hasStatus(400);
    ...
    void serialisesEveryLineOfAMultiLineOrder() {
        ...
        UUID orderId = UUID.randomUUID();
        UUID ownerId = OrderingFixtures.OWNER_ID;
        Order twoLineOrder = Order.rehydrate(
                orderId,
                ownerId,
                "EXT-MULTI",
                OrderingFixtures.deliveryTerms(),
                List.of(
    ...
    void exposesFulfillmentStateAndTimestamp() {
        ...
        Order fulfilled = Order.rehydrate(
                ...
                "EXT-FULFILLED",
                OrderingFixtures.deliveryTerms(),
                List.of(OrderLine.create(UUID.randomUUID(), 1, OrderingFixtures.OWNER_ID, SKU, 3)),
        ...
        response.bodyJson().extractingPath("$[0].fulfilledAt").isEqualTo(fulfilledAt.toString());
    }
    ...
    void carriesOwnerIdButNotOwnerName() {
        ...
        response.bodyJson().extractingPath("$[0].ownerId").isEqualTo(OrderingFixtures.OWNER_ID.toString());
    ...
    void shouldNotPlaceOrderForNonPostMethods() {
        assertThat(mvc.method(HttpMethod.PUT)
                        .uri("/orders")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(PLACE_ORDER_BODY))
                .hasStatus(405);
