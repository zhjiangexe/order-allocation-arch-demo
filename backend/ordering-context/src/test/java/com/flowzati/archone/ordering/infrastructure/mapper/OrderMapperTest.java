...
class OrderMapperTest {
    ...
    private static final UUID OWNER_ID = OrderingFixtures.OWNER_ID;
    private static final UUID FACILITY_ID = OrderingFixtures.FACILITY_ID;
    ...
    void mapsDomainToEntity() {
        Order order =
                OrderingFixtures.backorderedOrder(ORDER_ID, OWNER_ID, "SKU-1", 3, RECEIVED_AT, BACKORDERED_AT, 7L);
    ...
    void mapsEntityToDomainWithoutProducingDomainEvents() {
        ...
        assertThat(order.getDeliveryTerms())
                .isEqualTo(new DeliveryTerms(
                        FACILITY_ID,
                        "100",
                        "台北市中正區重慶南路一段 122 號",
                        LocalDate.of(2026, 8, 1),
    ...
    void roundTripsATwoLineOrder() {
        Order order = Order.rehydrate(
                ...
                List.of(
                        OrderLine.create(UUID.randomUUID(), 1, OWNER_ID, "SKU-1", 3),
                        OrderLine.create(UUID.randomUUID(), 2, OWNER_ID, "SKU-2", 7)),
                OrderStatus.PENDING,
                RECEIVED_AT,
                null,
                null,
        ...
        assertThat(restored.getLines()).extracting(OrderLine::getSkuCode).containsExactly("SKU-1", "SKU-2");
